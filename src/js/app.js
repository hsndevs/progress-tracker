...
function setCurrentMonth() {
	const monthSelect = document.getElementById('monthName');
	if (monthSelect) {
		const monthNames = [
...
async function saveProgressEntry() {
	...
	try {
		...
		if (submitButton) {
			...
			submitButton.innerHTML = '<i class="fas fa-save me-2"></i>Save Progress';
		}
...
window.editEntry = function (id) {
	...
	document.getElementById('editMonthName').value = entry.monthName || '';
...
async function saveEditedEntry() {
	...
	try {
		...
		const updatedData = {
			...
			phone: newPhone,
			email: newEmail,
			monthName: document.getElementById('editMonthName').value,
			startDate: today, // Keep today's date when editing
			dueDate: today,   // Keep today's date when editing
